 ...
 * @return bool result
 */
function xmldb_local_msm_upgrade($oldversion) {
    global $DB, $USER, $SITE;
    ...
    if ($result && $oldversion < 2019000007) {
        if (!$dbman->table_exists('msm_logs')) {
            $dbman->install_one_table_from_xmldb_file(__DIR__.'/install.xml', 'msm_logs');
        }
        upgrade_plugin_savepoint($result, '2019000007', 'local', 'o365');
    }

    if ($oldversion < 2019000008) {

        $dbman = $DB->get_manager();
        $table = new xmldb_table('msm_logs');

        $field = new xmldb_field('type', XMLDB_TYPE_CHAR, '255', null, null, null, '0', 'amount');

        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
    }
    if ($oldversion < 2019000009) {

        $dbman = $DB->get_manager();
        $table = new xmldb_table('msm_logs');

        $field = new xmldb_field('data', XMLDB_TYPE_CHAR, '255', null, null, null, '0', 'type');
    ...
    if ($oldversion < 2019000010) {

        $dbman = $DB->get_manager();
        $table = new xmldb_table('msm_logs');

        $field = new xmldb_field('quechannel', XMLDB_TYPE_INTEGER, '10', 0, XMLDB_NOTNULL, 0, '0', 'data');
    ...
    if ($oldversion < 2019000021) {


        // Adding fields to table local_ned_controller.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '11', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('type', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        $table->add_field('data1', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        $table->add_field('data2', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        $table->add_field('data3', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        $table->add_field('datajson', XMLDB_TYPE_INTEGER, '2', null, null, null, '0');
        ...
        upgrade_plugin_savepoint(true, 2019000021, 'local', 'msm');

    }

    if ($oldversion < 2019000029) {

        // Define table local_ned_controller to be created.
        $table = new xmldb_table('msm_datacache');

        $field = new xmldb_field('data4', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $field = new xmldb_field('data5', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $field = new xmldb_field('data6', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $field = new xmldb_field('data7', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $field = new xmldb_field('data8', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $field = new xmldb_field('data9', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $field = new xmldb_field('data10', XMLDB_TYPE_CHAR, '255', null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
